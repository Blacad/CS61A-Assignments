...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if hasattr(self,'damage') and not hasattr(self,'doubled'):
            self.damage *= 2
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while p and not p.is_hive:
            if self.lower_bound <= distance <= self.upper_bound and random_bee(p.bees):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        # 因为bee是可变类型赋值过来是浅拷贝，因此切片虽然是新实例但是里面的元素都是指向老bee们
        # 所以可以直接操作
        ...
        while p:
            if p.ant:
                ...
                if p.ant.is_container and p.ant.ant_contained:
                    ...
                    p.ant.ant_contained.doubled = True
            p = p.exit
        # END Problem 12
